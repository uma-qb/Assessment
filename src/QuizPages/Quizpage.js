...
function Quizpage() {
    const { userData, selectedSection, selectedGroup, setQuestions, setSelectedGroup, setSelectedSection } = useContext(UserContext);
    const [sections, setSections] = useState([])
    // const [selectedSection, setSelectedSection] = useState(UserContext);
        ...
        if (selectedSection !== null) {
            ...
            const fetchQuestions = async () => {
                try {
                    ...
                    const responseQuestions = await axios.post(`http://127.0.0.1:8000/api/quiz_attempt/${token}`);
                                ...
                                <Accordion defaultActiveKey="0" flush>
                                    {sections?.map((sectionGroup, index) => (
                                        <Accordion.Item eventKey={`${index}`} key={sectionGroup - index}>
                                            <Accordion.Header key={sectionGroup-index}><b>{sectionGroup.category}</b></Accordion.Header>
                                            <Accordion.Body>
                                                {sectionGroup.sections.map((section, idx) => (
                                                    <div className='w-100 m-2'>
                                                        <label key={`${section.cid-idx}`} htmlFor={`inline-radio-${section.cid}`} style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                                                            <Form.Check
                                                                inline
                                                                name="group1"
                                                                type="radio"
